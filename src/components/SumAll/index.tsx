    ...
    return(
        <div className="flex lg:flex-row flex-col w-full justify-between border border-solid border-slate-200 pr-2 lg:pt-4 pt-2">  
            ...
            <div className="flex flex-col items-start lg:w-1/2 w-full">
                <div className="mb-2 rounded-2xl bg-blue-100 py-1 pr-2 pl-2 w-full text-base">{items[0]} - {sum(expenses, "Food")} </div>
                <div className="mb-2 rounded-2xl bg-violet-100 py-1 pr-2 pl-2 w-full text-base">{items[1]} - {sum(expenses, "Entertainment")}</div>
                <div className="mb-2 rounded-2xl bg-pink-100 py-1 pr-2 pl-2 w-full text-base">{items[2]} - {sum(expenses, "Education")}</div>
                <div className="mb-2 rounded-2xl bg-yellow-100 py-1 pr-2 pl-2 w-full text-base">{items[3]} - {sum(expenses, "Rent")}</div>
                <div className="mb-2 rounded-2xl bg-green-100 py-1 pr-2 pl-2 w-full text-base">{items[4]} - {sum(expenses, "Clothes")}</div>
                <div className="mb-2 rounded-2xl bg-slate-100 py-1 pr-2 pl-2 w-full text-base text-red-600">Total - {totalSum(expenses)}</div>
            </div>
        </div>
    )
}
